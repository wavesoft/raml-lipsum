...
var RAMLGenerator = function( ramlPath ) {
	...
	function getTypeRef(typeName) {
		if (typeName.includes('|')) {
			return getUnionTypeRef(typeName)
		}
	...
	/**
	 * Resolves a union type name to a type object
	 */
	function getUnionTypeRef(typeName) {
	...
	/**
	 * Call the given callback to generate array at random interval
	 */
	function randomArray(minItems, maxItems, callback) {
		var rand = Math.round(Math.random() * (maxItems - minItems)) + minItems;
		return Array(rand, null).map(callback);
	}

	/**
	 * Generate a random data between a range
	 */
	...
	function randomSingleDataForNative(type) {
		var d = randomDate(new Date(2015, 0, 1), new Date());
		...
		switch (typeName) {
			...
			case 'integer':
				...
				if (type.minimum) min = type.minimum();
				if (type.maximum) max = type.maximum();
			...
			case 'string':
				...
				if (type.enum) enumValues = type.enum();
				if (type.pattern) pattern = type.pattern();
	 ...
	 * (This function also supports array native types)
	 */
	function randomDataForNative(type) {
		var typeName = normalizeTypeName(type.type()[0]);
		if (typeName.endsWith('[]')) {
			return randomArray(0, 10, function() {
				return randomSingleDataForNative(type);
			});
		} else {
			return randomSingleDataForNative(type);
		}
	...
	function randomArrayDataFor(type) {
		...
		return randomArray(minItems, maxItems, function() {
